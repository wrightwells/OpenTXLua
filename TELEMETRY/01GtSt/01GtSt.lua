...
local MaxBatteryVoltage = 4.2
local MinBatteryVoltage = 3.4
local x = 0
local y = 0
local fh = 20
...
local function run_func(event)
  ...
  lcd.drawNumber(x, y,  ReceiverBatteryVoltage_startup *10, PREC1)
  lcd.drawText( lcd.getLastPos(), y, "V at start up.")
  ...
  y = y + fh
  if ReceiverBatteryVoltage_startup > 0 then
    lcd.drawGauge(x, y, LCD_W-1, 2 * fh, ReceiverBatteryVoltage_current*10, MaxBatteryVoltage*10)
    --lcd.drawGauge(x, y, LCD_W-1, 2 * fh, ReceiverBatteryVoltage_current*10, ReceiverBatteryVoltage_startup*10)
  else
    lcd.drawText (x,y, "Quadcopter not on")
  end
  ...
  lcd.drawChannel(x,y, BatterySensor, XXLSIZE)
  -- else
